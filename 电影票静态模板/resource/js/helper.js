...
function verifyautologin() {
    if (getCookie("Mobile") == null) {
        ...
        if (isnull(Mobile) || isnull(Token))
            return;
        $.ajax({
            url: 'account/VerifyAutoLogin.html',
            type: 'post',
            data: { Mobile: Mobile, Token: Token },
            success: function (respData) {
                if (respData.ResultCode == "0") {
                    SetCookie("LocalToken", respData.ErrMsg, 7);
                    SetCookie("LocalMobile", respData.Mobile, 7);
                    setsessionstorage(respData);
                    $(".tip").empty();
                    $(".tip").html('欢迎来到江西电影票，<span class="user-name"></span><a class="loginout" href="javascript:void(0);">退出</a>');
...
function checkUrl(url) {
    ...
    if (pagename.indexOf("film") > -1) k = "film";
    if (pagename.indexOf("event") > -1) k = "event";
    if (pagename.indexOf("news") > -1) k = "news";
    if (pagename.indexOf("gyk") > -1) k = "gyk";
